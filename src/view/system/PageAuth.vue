    ...
    <div class="container">
      ...
      <main>
        ...
        >
          <template #header>
            <div>
              <el-checkbox v-model="menus[0].checked">
                <span :class="menus[0].extra.split('|')[1]">{{ menus[0].extra.split('|')[0] }}</span>
              </el-checkbox>
            </div>
          </template>
          <div class="card-main">
            <div
              ...
              :key="menu.c_id"
            >
              <el-checkbox v-model="menu.c_checked">
                {{ menu.c_extra }}
              </el-checkbox>
            </div>
          </div>
        ...
        >
          ...
          <div class="card-main">
            <div
              ...
              :key="method.c_id"
            >
              <el-checkbox v-model="method.c_checked">
                {{ method.c_extra }}
              </el-checkbox>
            </div>
          </div>
    ...
    >
      <el-form label-width="12em">
        <el-form-item :label="'创建新的父级'+addTypeName">
          <el-switch
            v-model="addDialog.addParent"
            @change="clearParent"
          />
        </el-form-item>
        <el-form-item
          v-if="addDialog.addParent"
          :label="'父' + addTypeName + '显示名称'"
        >
          <el-input v-model="addDialog.data.parent.extra" />
        </el-form-item>
        <el-form-item
          v-if="addDialog.addParent"
          :label="'父' + addTypeName + '地址'"
        >
          <el-input v-model="addDialog.data.parent.target" />
        </el-form-item>
        <el-form-item
          v-else
          :label="'请选择父级'+addTypeName"
        >
          <el-select v-model="addDialog.data.parent.id">
            <el-option
              v-for="opt in addDialog.options"
              :key="opt.id"
              :value="opt.id"
              :label="opt.extra"
            />
        ...
        <el-form-item :label="'子' + addTypeName + '显示名称'">
          <el-input v-model="addDialog.data.child.extra" />
        </el-form-item>
        <el-form-item :label="'子' + addTypeName + '地址'">
          <el-input v-model="addDialog.data.child.target" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button
